...
was migrated from the current Power Query evaluation timestamp to persistent automation state.

Validated behavior:

- before the first production run: not evaluated;
- successful production run: PASS with persistent timestamp;
- later failed run: previous successful timestamp preserved;
- subsequent successful recovery: timestamp advances.
...
was migrated from N/A to persistent automation-controlled evaluation.

Validated behavior:

- before workflow execution: `NOT_EVALUATED`;
- successful PivotTable stage: `PASS`;
- workflow failure before PivotTable stage: `NOT_EVALUATED`.
...
`vba/modules/modAutomationState.bas`

Validated behavior:

- configuration validation;
- state lookup;
- controlled transition to `RUNNING`;
- PivotTable status validation;
- rejection of false successful completion when Pivot refresh is not PASS;
- controlled failure-state persistence;
- preservation of `LastSuccessfulRefresh` after failure.

Controlled safety test attempted successful completion while:

`PivotRefreshStatus = NOT_EVALUATED`

Observed:

successful completion was rejected.
...
- `.Refresh` returned control almost immediately;
- target QueryTable remained `Refreshing = True`.
...
Excel remained blocked for several minutes and required forced workbook recovery.
...
the eight target QueryTables remained refreshing until the 900-second diagnostic timeout.

All active QueryTables were then cancelled.
...
| Query | Approx. Duration | Rows | Result |
|---|---:|---:|---|
| `dim_Product` | 2.01 seconds | 300 | PASS |
| `fact_PurchaseOrders` | 6.93 seconds | 29,666 | PASS |
| `fact_InventoryWeekly` | 97.37 seconds | 280,800 | PASS |

All three validations returned:

- refresh result TRUE;
- `Refreshing = False` after completion;
- expected loaded row count.
...
Each target was validated against its physical worksheet, Excel Table and workbook connection before production execution.
...
`vba/modules/modQualityControl.bas`

Validated evaluation levels:

- Preliminary QC excludes `QC-030` and `QC-032`;
- Post-Pivot QC excludes only `QC-030`;
- Full QC evaluates all controls.

Baseline before production automation:

- Preliminary QC: PASS
- Post-Pivot QC: WARNING
- Full QC: WARNING

This confirmed that the automation-dependent controls prevented a false overall PASS before production execution.
...
`vba/modules/modPivotRefresh.bas`

Validated analytical structure:

- PivotCaches: 5
- Inventory PivotTables: 3
- Procurement PivotTables: 4
- Supplier PivotTables: 3
- Total analytical PivotTables: 10

Production behavior:

- each unique PivotCache refreshed once;
- all 10 PivotTables updated;
- module itself does not assert persistent `QC-032` success.

After controlled Pivot refresh outside the full workflow:
...
`vba/modules/modAutomation.bas`

Validated sequence:

1. configuration validation;
2. automation state begins as `RUNNING`;
3. controlled Power Query refresh;
4. Excel full calculation;
5. preliminary Quality Control;
6. PivotCache / PivotTable refresh;
7. persistent `QC-032 = PASS`;
8. post-Pivot calculation;
9. post-Pivot Quality Control;
10. accepted automation-state finalization;
11. final calculation;
12. full Quality Control;
13. user-facing completion message.
...
The initial orchestrator incorrectly passed this Quality Control value directly to the automation-state finalizer, which correctly accepts only:
...
The workflow was correctly marked:

`FAILED`

and did not record a false successful refresh.

The orchestrator was corrected to map:

- Quality Control `PASS` → Automation `SUCCESS`
- Quality Control `WARNING` → Automation `WARNING`

The corrected workflow was re-executed successfully.
...
- `LastErrorDescription` blank
- Full Quality Control: PASS
- `QC-030`: PASS
- `QC-032`: PASS
- Overall Quality Status: PASS

A persistent `LastSuccessfulRefresh` timestamp was recorded.
...
`parts_master.csv` was temporarily renamed so that the required Products source could not be found.

The production workflow was executed.
...
The previous successful refresh timestamp remained:

`2026-09-22 16:44:06`

The source file was then restored to its original name.
...
the production workflow was executed again.
...
- `LastErrorDescription` blank
- Full Quality Control: PASS
- new `LastSuccessfulRefresh = 2026-09-22 17:15:38`

The workflow therefore recovered from the prior controlled failure, cleared obsolete failure evidence and advanced the successful-refresh timestamp only after a valid run.
...
- a failed current attempt preserves the previous successful timestamp;
- current Quality Control changes to FAIL;
- previous outputs are therefore not presented as current successful output.

A Form Control button:

`Refresh ProcureFlow`

was added to `00_HOME`.

The button invokes the same production entry point:

`RefreshProcureFlow`

and completed successfully.

Observed end-to-end duration from the user-facing button:
...
Power Query orchestration:

PASS

PivotTable orchestration:
...
Stale-output visibility:

PASS

User-facing refresh entry point:
